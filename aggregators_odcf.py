...
class Aggregator(object):
    ...
    def __call__(self, self_vectors, neighbor_vectors, neighbor_relations, context, masks):
        outputs = self._call(self_vectors, neighbor_vectors, neighbor_relations, context, masks)
        return outputs

    @abstractmethod
    def _call(self, self_vectors, neighbor_vectors, neighbor_relations, context, masks):
        # dimension:
    ...
    def _mix_neighbor_vectors(self, self_vectors, neighbor_vectors, neighbor_relations, context):
        avg = False
        if not avg:
            # [batch_size, 1, 1, dim]
            shape_list = neighbor_relations.get_shape().as_list()
            context = tf.reshape(context, [self.batch_size, -1,1, self.dim])
            neighbor_relations = tf.reshape(neighbor_relations, [self.batch_size, -1, shape_list[2], self.dim])
            att_tan = neighbor_vectors + neighbor_relations
            # [batch_size, -1, n_neighbor]
            user_relation_scores = tf.reduce_sum(context * att_tan, axis=-1)
            user_relation_scores_normalized = tf.nn.softmax(user_relation_scores, dim=-1)
            z = user_relation_scores_normalized
...
class SumAggregator(Aggregator):
    ...
    def _call(self, self_vectors, neighbor_vectors, neighbor_relations, context, masks):
        # [batch_size, -1, dim]
        neighbors_agg, self.z = self._mix_neighbor_vectors(self_vectors, neighbor_vectors, neighbor_relations, context)
...
class ConcatAggregator(Aggregator):
    ...
    def _call(self, self_vectors, neighbor_vectors, neighbor_relations, context, masks):
        # [batch_size, -1, dim]
        neighbors_agg = self._mix_neighbor_vectors(self_vectors, neighbor_vectors, neighbor_relations, context)
